...
def cost_curves(BCET, BCSC, MEWD, MEWG, MEWL, MEPD, MERC, MRCL, RERY, MPTR, MRED, MRES, rti, t2ti, erti, year, dt):
    ...
    tech_to_resource = [0, 1, 2, 2, 2, 2, 3, 3, 4, 4, 4, 4, 5, 6, 7, 8, 9, 10, 11, 11, 12, 13, 3, 3,4,4,4]
    ...
    for r in range(len(rti)):           # Loop over region
        for j in range(len(t2ti)): 
            ...
            if(MEPD[r, tech_to_resource[j]] > 0.0):
                ...
                if BCET[r, j, 13] != 0 and not np.isnan(BCET[r, j, 13]):
                    BCET[r, j, 4] = BCET[r, j, 4] + \
                        (MERC[r, tech_to_resource[j], 0] - MRCL[r, tech_to_resource[j], 0]) * 3.6 / BCET[r, j, 13] 
                else:
                    print(f"Warning: Division by zero or NaN detected at region {r}, technology {j}. Skipping this calculation.")
